...
francis_t = define_character("Мысли Миллера", color="#ffccss")
...
def no_drinks():
    ...
    hide_character("bartender")
    show_scene("street", effect="dissolve")
    scene5()
...
def scene5():
    ...
    # "Лучше опоздать"
    say(None, "if choice_result == 'late_arrival'")
    late()

def yearly():
    show_scene("black",effect="Fade")
    say(narrator,
    ...
    say(narrator,
        "Он постучал в дверь, и вскоре услышал шум изнутри — глухие звуки, которые напоминали падение чего-то тяжёлого.")
    show_scene("alan_home", effect="dissolve")
    say(narrator,
        "Миллер насторожился, но, всё же, открыл дверь. На пороге стоял Алан, его лицо было бледным, а глаза полны тревоги. ")
    say(narrator, "Он выглядел на грани, словно только что пережил бурю.")
    say(alan,"Миллер, заходи.")

    show_scene("alan_inside", effect="Dissolve")
    say(narrator,
        "Френсис вошёл в квартиру, и его встретила атмосфера, наполненная страхом. Стены были обиты тёмным деревом, а полы устланы выцветшими коврами, которые помнили лучшие времена")

    say(narrator, "На столе, заваленном книгами и бумагами, стоял чайник, а вокруг валялись чашки, некоторые из которых были перевернуты.")

    say(narrator, "В углу комнаты стоял рояль, на котором лежали разложенные ноты, а рядом с ним — стул, который, похоже, только что упал.")
    ...
    say(narrator, "Алан кивнул, но его нервозность становилась всё более заметной. ")
    say(narrator,"Он уселся за стол, его руки дрожали, а взгляд метался по комнате, как будто он искал спасение в каждом предмете вокруг. ")
    say(narrator,"Миллер заметил, как Алан начал мямлить, оправдываясь за свои эмоции и неуверенность.")
    say(alan, "Извини, я просто… не знаю, с чего начать.")
    say(alan," Всё это… слишком много.")
    say(francis, "Давай просто поговорим нормально. Что ты конкретно хочешь мне рассказать?")
    say(alan,"Миллер, послушай, меня считают убийцей, но я не убивал его. Это ужасно, и я просто не понимаю, как всё это произошло.")
    say(narrator,"Он глубоко вздохнул, стараясь собрать свои мысли в единое целое.")
    say(alan, "Я был с Кассиусом в тот день. Мы случайно встретились возле бара.")
    qna()

def late():
    show_scene("black",effect="Fade")
    say(narrator,
        "Он решил последовать велению своего товарища, проведя ещё какое-то время в ожидании, Френсис наконец отправился. Поднимаясь по старой деревянной лестнице, "
        "он заметил, как половицы скрипят под его ногами, создавая странное ощущение, будто дом сам шептал свои тайны.")
    say(narrator,
        "В воздухе витал запах свежезаваренного чая и чего-то сладкого, но с каждым шагом его охватывало чувство, что что-то не так.")
    show_scene("alan_home", effect="dissolve")
    say(narrator,
        "Стоя перед дверью, кисть Френсиса сжалась. Поднеся тыльную сторону руки к двери, Френсис услышал шум из замочной скажены двери.")
    show_scene("alan_home", effect="hpunch")
    say(narrator, "Отойдя назад, он немного подождал, пока копошившийся внутри человек не закончит возиться с замком.")
    show_scene("alan_home", effect="hpunch")
    say(narrator,"Наконец дверь открылась.")
    say(narrator,"Изнутри послышался робкий, но всё ещё такой же звенящий как раньше голос.")

    say(alan, "Миллер, заходи.")
    show_scene("alan_inside", effect="Dissolve")
    show_character("francis")
    say(francis, "Здоров.")
    say(narrator,
        "Френсис вошёл в квартиру. Стены были обиты тёмным деревом, а полы устланы выцветшими коврами, которые помнили лучшие времена")

    say(narrator, "В углу комнаты стоял рояль, а рядом с ним — стул. Всё это нежно окутывало закатное солнце. Своим теплым светом, оно добовляло уюта, этой неприветливой комнате.")
    say(narrator, "Осматривая комнату дальше, Взгляд Френсиса зацепили стол и скудная пара стульев.")
    say(francis, "Давай сядем и поговорим.")
    say(narrator, "Алан кивнул, но его нервозность становилась всё более заметной. ")
    say(narrator,
        "Он уселся за стол, его руки дрожали, а взгляд метался по комнате, как будто он искал спасение в каждом предмете вокруг. ")
    say(narrator, "Миллер заметил, как Алан начал мямлить, оправдываясь за свои эмоции и неуверенность.")
    show_character("alan")
    say(alan, "Извини, я просто… не знаю, с чего начать.")
    say(alan, " Всё это… слишком много.")
    say(francis, "Давай просто поговорим нормально. Что ты конкретно хочешь мне рассказать?")
    say(alan,
        "Миллер, послушай, меня считают убийцей, но я не убивал его. Это ужасно, и я просто не понимаю, как всё это произошло.")
    say(narrator, "Он глубоко вздохнул, стараясь собрать свои мысли в единое целое.")
    say(alan, "Я был с Кассиусом в тот день. Мы случайно встретились возле бара.")
    qna()
